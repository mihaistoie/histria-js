...
class Transaction {
    ...
    emitInstanceEvent(eventType, eventInfo, instance, ...args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            args = args || [];
            let nIstances = [ci];
            let pi = eventType === interfaces_1.EventType.propChanged && args && args.length ? 0 : -1;
            let list = that._subscribers.get(eventType);
            if (list) {
                while (ci) {
                    for (let item of list)
                        yield item(eventInfo, ci.constructor, nIstances, args);
                    if (eventType !== interfaces_1.EventType.propChanged)
                        break;
    ...
    findOne(filter, classOfInstance) {
        return __awaiter(this, void 0, void 0, function* () {
            let that = this;

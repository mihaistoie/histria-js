...
class Transaction {
    ...
    loadFromJson(data) {
        ...
        data && data.instances.forEach((item) => {
            ...
            that._restore(factory, item.data);
        });
    }
    ...
    create(classOfInstance) {
        return __awaiter(this, void 0, void 0, function* () {
            let that = this;
            let mm = model_manager_1.modelManager();
            let instance = mm.createInstance(classOfInstance, this, null, '', { $isNew: true }, { isRestore: false });
    ...
    find(classOfInstance, filter, options) {
        return __awaiter(this, void 0, void 0, function* () {
            let that = this;
            let res = yield that._find(filter, classOfInstance);
            if (!options || !options.onlyInCache) {
                let store = that._store(classOfInstance);
                if (store) {
                    let list = yield store.find(classOfInstance.entityName, filter, { compositions: false });
                    if (list && list.length) {
                        let map = {};
                        let instances = that._getInstances(classOfInstance);
                        let promises = list.map(item => {
                            let o = instances ? instances.get(item.id + '') : null;
                            map[item.id + ''] = true;
                            return o ? Promise.resolve(o) : that.load(classOfInstance, item);
                        });
                        let dbList = yield Promise.all(promises);
    ...
    findOne(classOfInstance, filter, options) {
        return __awaiter(this, void 0, void 0, function* () {
            let that = this;
            ...
            if (!options || !options.onlyInCache) {
                let store = that._store(classOfInstance);
                if (store) {
                    let data = yield store.findOne(classOfInstance.entityName, filter, { compositions: true });
                    if (data) {
                        return yield that.load(classOfInstance, data);
                    }
                }
            }
            return null;
    ...
    removeInstance(classOfInstance, instance) {
        let that = this;
        if (that._instances) {

...
function _genSchemaProperties(schema, code, model) {
    Object.keys(schema.properties || {}).forEach(propertyName => {
        ...
        if (histria_utils_1.schemaUtils.isHidden(propSchema) || histria_utils_1.schemaUtils.isComplex(propSchema))
            return;
        let stype = histria_utils_1.schemaUtils.typeOfProperty(propSchema);
        let isReadOnly = histria_utils_1.schemaUtils.isReadOnly(propSchema);
        switch (stype) {
            case histria_utils_2.JSONTYPES.string:
                ...
                break;
            case histria_utils_2.JSONTYPES.boolean:
                ...
                break;
            case histria_utils_2.JSONTYPES.id:
                ...
                code.push(_tab(1) + '}');
                break;
            case histria_utils_2.JSONTYPES.integer:
                ...
                break;
            case histria_utils_2.JSONTYPES.number:
...
function _genVewRelations(schema, code, model) {
    schema.relations && Object.keys(schema.relations).forEach(relationName => {
        ...
        switch (relation.type) {
            case histria_utils_2.RELATION_TYPE.hasOne:
                ...
                code.push(_tab(1) + '}');
                break;
            case histria_utils_2.RELATION_TYPE.belongsTo:
                code.push(_tab(1) + util.format('public %s(): Promise<%s> {', relationName, refClass));
                code.push(_tab(2) + util.format('return this._children.%s.getValue();', relationName));
                code.push(_tab(1) + '}');
                code.push(_tab(1) + util.format('public set%s(value: %s): Promise<%s> {', _upperFirstLetter(relationName), refClass, refClass));
                ...
                code.push(_tab(1) + '}');
                break;
            case histria_utils_2.RELATION_TYPE.hasMany:
    ...
    schema.relations && Object.keys(schema.relations).forEach(relationName => {
        ...
        switch (relation.type) {
            ...
            case histria_utils_2.RELATION_TYPE.belongsTo:
                ...
                break;
            case histria_utils_2.RELATION_TYPE.hasMany:
                const cn = (relation.aggregationKind === histria_utils_2.AGGREGATION_KIND.shared) ? 'HasManyAggregation' : 'HasManyComposition';
...
function _genClassErrors(schema, className, baseClass, code) {
    ...
    schema.relations && Object.keys(schema.relations).forEach(relName => {
        ...
        switch (relation.type) {
            case histria_utils_2.RELATION_TYPE.hasMany:
                generateError = (relation.aggregationKind === histria_utils_2.AGGREGATION_KIND.composite);
                break;
            case histria_utils_2.RELATION_TYPE.hasOne:
                generateError = (relation.aggregationKind !== histria_utils_2.AGGREGATION_KIND.composite);
                break;
        ...
        if (propSchema.enum) {
            ...
            switch (stype) {
                ...
                case histria_utils_2.JSONTYPES.number:
                    ...
                    code.push(_tab(1) + '}');
                    break;
                case histria_utils_2.JSONTYPES.date:
                    code.push(_tab(1) + util.format('public get %s(): DateState {', propName));
                    code.push(_tab(2) + util.format('return this._states.%s;', propName));
                    code.push(_tab(1) + '}');
                    break;
                case histria_utils_2.JSONTYPES.datetime:

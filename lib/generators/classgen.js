...
function generate(codeByClass, schema, baseClass, pathToLib) {
    ...
    Object.keys(schema.properties || {}).forEach(propName => {
        ...
        if (schemaUtils.isHidden(propSchema))
            return;
        let stype = schemaUtils.typeOfProperty(propSchema);
        if (stype == schema_consts_1.JSONTYPES.array || stype == schema_consts_1.JSONTYPES.object)
            return;
        if (propSchema.enum) {

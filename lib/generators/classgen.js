...
async function saveCode(codeByClass, codeByNamespace, dstFolder) {
    ...
    let writersNamespaces = Object.keys(codeByNamespace).map(name => {
        ...
        return histria_utils_1.fs.writeFile(path.join(dstFolder, item.fileName + '.ts'), item.import.concat(item.code).join('\n'));
    });
...
function _generate(codeByClass, codeByNameSpace, model, pathToLib) {
    ...
    Object.keys(model).forEach((fullName) => {
        let schema = model[fullName];
        let isView = schema.view ? true : false;
...
function _genEmbeddedRelationsProperties(schema, code, model) {
    schema.relations && Object.keys(schema.relations).forEach(relationName => {
        const relation = schema.relations[relationName];
        if (relation.embedded && relation.type === histria_utils_3.RELATION_TYPE.hasOne && relation.aggregationKind === histria_utils_3.AGGREGATION_KIND.composite) {
...
function _genEmbedddedSchemaProperties(schema, parentSchema, relationName, code) {
    const parentProps = parentSchema.properties || {};
    Object.keys(schema.properties || {}).forEach(propertyName => {
        if (parentProps[propertyName])
            return;
        const propSchema = schema.properties[propertyName];
        if (histria_utils_2.schemaUtils.isHidden(propSchema) || histria_utils_2.schemaUtils.isComplex(propSchema))
            return;
        const stype = histria_utils_2.schemaUtils.typeOfProperty(propSchema);
        const isReadOnly = histria_utils_2.schemaUtils.isReadOnly(propSchema);
        switch (stype) {
            case histria_utils_3.JSONTYPES.string:
                code.push(_tab(1) + util.format('public get %s(): string {', propertyName));
    ...
    schema.relations && Object.keys(schema.relations).forEach(relationName => {
        ...
        switch (relation.type) {
            ...
            case histria_utils_3.RELATION_TYPE.hasMany:
                const cn = (relation.aggregationKind === histria_utils_3.AGGREGATION_KIND.shared) ? 'HasManyAggregation' : 'HasManyComposition';
...
function _genClassErrors(schema, className, baseClass, code) {
    ...
    schema.relations && Object.keys(schema.relations).forEach(relName => {
        ...
        switch (relation.type) {
            case histria_utils_3.RELATION_TYPE.hasMany:
                generateError = (relation.aggregationKind === histria_utils_3.AGGREGATION_KIND.composite);
                break;
            case histria_utils_3.RELATION_TYPE.hasOne:
                generateError = (relation.aggregationKind !== histria_utils_3.AGGREGATION_KIND.composite);
                break;
...
async function classGenerator(srcFolder, dstFolder, pathToLib) {
    let model = {};
    await histria_utils_2.schemaUtils.loadModel(srcFolder, model);
    let codeByClass = {};
    let codeByNamespace = {};

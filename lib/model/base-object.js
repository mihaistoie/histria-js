...
class InstanceErrors {
    constructor(parent, schema) {
        ...
        that._schema = schema;
        that._parent = parent;
        that._messages.$ = new error_1.Error(that._parent, '$');
        schema && schema.properties && Object.keys(schema.properties).forEach(propName => {
            that._messages = new error_1.Error(that._parent, propName);
        });
    }
    destroy() {
        let that = this;
...
class Instance {
    constructor(transaction, parent, parentArray, propertyName, value, options) {
        let that = this;
        that._status = instance_1.ObjectStatus.idle;
    ...
    _canExecutePropChangeRule() {
        let that = this;
        let root = that.getRoot();
        return root._status === instance_1.ObjectStatus.idle;
    }
    _createProperties() {
        let that = this;

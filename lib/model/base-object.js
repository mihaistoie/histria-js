...
const roleHasMany_1 = require("./roleHasMany");
...
class Instance {
        ...
        that._schema && that._schema.properties && Object.keys(that._schema.properties).forEach(propName => {
            ...
            switch (propType) {
                case schema_consts_1.JSONTYPES.integer:
                    ...
                    break;
                case schema_consts_1.JSONTYPES.id:
            ...
            switch (relation.type) {
                ...
                case schema_consts_1.RELATION_TYPE.hasMany:
                    if (relation.aggregationKind === schema_consts_1.AGGREGATION_KIND.none) {
                        ...
                        //aggregation
                        that._children[relName] = new roleHasMany_1.HasManyAggregation(that, relName, relation);
                    }
                    else if (relation.aggregationKind === schema_consts_1.AGGREGATION_KIND.composite) {
                        //composition
                        that._children[relName] = new roleHasMany_1.HasManyComposition(that, relName, relation, that._model[relName]);
                    }
                    break;
            }
        });
    ...
    isArrayComposition(propName) {
        ...
        if (that._schema.relations && that._schema.relations[propName]) {
            ...
            return rel.type === schema_consts_1.RELATION_TYPE.hasMany && rel.aggregationKind === schema_consts_1.AGGREGATION_KIND.composite;
        }
    }
    ...
    getPropertyByName(propName, value) {
        let that = this;
        let propSchema = that._schema.properties[propName];
        let mm = new model_manager_1.ModelManager();
        if (!propSchema)

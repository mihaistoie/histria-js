...
class InstanceState {
    constructor(parent, schema) {
        ...
        schema && schema.properties && Object.keys(schema.properties).forEach(propName => {
            ...
            if (cs.enum) {
                that._states[propName] = new state_1.EnumState(that._parent, cs, propName);
            }
            else {
                switch (propType) {
                    case schema_consts_1.JSONTYPES.integer:
                        that._states[propName] = new state_1.IntegerState(that._parent, cs, propName);
                        break;
                    case schema_consts_1.JSONTYPES.number:
                        that._states[propName] = new state_1.NumberState(that._parent, cs, propName);
                        break;
                    case schema_consts_1.JSONTYPES.date:
                        that._states[propName] = new state_1.DateState(that._parent, cs, propName);
                        break;
                    case schema_consts_1.JSONTYPES.datetime:
                        that._states[propName] = new state_1.DateTimeState(that._parent, cs, propName);
                        break;
                    ...
                    case schema_consts_1.JSONTYPES.refobject:
                        that._states[propName] = new state_1.RefObjectState(that._parent, cs, propName);
                        break;
                    case schema_consts_1.JSONTYPES.refarray:
                        that._states[propName] = new state_1.RefArrayState(that._parent, cs, propName);
                        break;
                    default:
                        that._states[propName] = new state_1.StringState(that._parent, cs, propName);
                        break;
                }
            }
        });

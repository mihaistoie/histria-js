...
class InstanceErrors {
    constructor(parent, schema) {
        ...
        that._schema = schema;
        that._parent = parent;
        that._messages.$ = new error_1.ErrorState(that._parent, '$');
        schema && schema.properties && Object.keys(schema.properties).forEach(propName => {
            that._messages[propName] = new error_1.ErrorState(that._parent, propName);
        });
    }
    destroy() {
        let that = this;
...
class Instance {
    constructor(transaction, parent, parentArray, propertyName, value, options) {
        ...
        that._status = interfaces_1.ObjectStatus.idle;
    ...
    createErrors() { }
    createStates() { }
    _canExecutePropChangeRules() {
        let that = this;
        let root = that.getRoot();
        return root._status === interfaces_1.ObjectStatus.idle;
    }
    _canExecuteValidateRules() {
        let that = this;
        let root = that.getRoot();
        return root._status === interfaces_1.ObjectStatus.idle;
    }

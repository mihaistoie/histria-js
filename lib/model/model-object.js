...
class ModelObject extends base_instance_1.BaseInstance {
    ...
    async viewOfMe(classOfView) {
        const that = this;
        let schema = histria_utils_1.schemaManager().schema(classOfView.nameSpace, classOfView.entityName);
        let cr = null;
        ...
        if (cr) {
            ...
            return that.transaction.findOneInCache(classOfView, query);
        }
        else
            return null;
    }
    ...
    async _viewFactory(hook, propName, op, source) {
        ...
        if (propName === hook.property) {
            ...
            if (models.length) {
                const classConstructor = model_manager_1.modelManager().classByNameAndPath(that._schema.name, that._schema.nameSpace, hook.property);

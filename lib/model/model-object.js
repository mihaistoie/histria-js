...
class ModelObject extends base_instance_1.BaseInstance {
    ...
    async viewOfMe(classOfView) {
        const that = this;
        let schema = histria_utils_1.schemaManager().schema(classOfView.nameSpace, classOfView.entity);
        let cr = null;
        ...
        if (cr) {
            const query = histria_utils_1.schemaUtils.roleToQueryInv(cr, that.model());
            const viewClass = model_manager_1.modelManager().classByName(cr.model, cr.nameSpace);
            return that.transaction.findOne(query, viewClass, { onlyInCache: true });
        }
        else
            return null;
    }

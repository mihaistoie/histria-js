...
class HasOneAC extends HasOne {
        ...
        await that._parent.changeProperty(that._propertyName, oldValue, value, () => {
            ...
            if (that._relation.invRel) {
                ...
                if (that._value) {
                    ...
                    if (!useInv) {
                        histria_utils_1.schemaUtils.updateRoleRefs(that._relation, fmodel, null, useInv);
                    }
                }
            }
        }, { isLazyLoading: false });
    ...
    async _lazyLoad() {
        ...
        if (query) {
            that._value = null;
            const opts = { onlyInCache: that.refIsPersistent };
...
class HasOneComposition extends HasOneAC {
    constructor(parent, propertyName, relation) {
        ...
        if (childModel === null)
            ...
            if (!isRestore) {
                const useInv = (that.refIsPersistent || that._parent.isPersistent);
                if (useInv)
                    histria_utils_1.schemaUtils.updateRoleRefs(that._relation, childModel, pmodel, useInv);
                else
                    histria_utils_1.schemaUtils.updateRoleRefs(that._relation, pmodel, childModel, useInv);
            }
            if (!isRestore && that._value) {
                parent.pushLoaded(() => that._notifyHooks(that._value, interfaces_1.EventType.addItem));
            }
        }
    }
    ...
    async _afterSetValue(newValue, oldValue) {
        ...
        if (oldValue)
            await that._notifyHooks(oldValue, interfaces_1.EventType.removeItem);
        if (newValue)
            await that._notifyHooks(newValue, interfaces_1.EventType.addItem);
    }
    ...
    async _setValue(value) {
        ...
        if (that._relation.aggregationKind === histria_utils_1.AGGREGATION_KIND.composite) {
            if (oldValue)
                await that._notifyHooks(oldValue, interfaces_1.EventType.removeItem);
            if (that._value)
                await that._notifyHooks(that._value, interfaces_1.EventType.addItem);
        }
    }
    async _lazyLoad() {
        const that = this;

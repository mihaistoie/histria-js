...
function idDefinition() {
    return { type: schema_consts_1.JSONTYPES.integer, generated: true };
}
function refIdDefinition() {
    return { type: schema_consts_1.JSONTYPES.integer, isReadOnly: true };
}
//reference :
// schema_name"
// schema_name.json"
...
function _checkRelations(schema, model) {
    schema.relations && schema.relations.forEach(relName => {
        ...
        if (!refRel && refRel.aggregationKind !== schema_consts_1.AGGREGATION_KIND.none) {
        ...
        if (refRel) {
            ...
            if (refRel.type === rel.type) {
                throw util.format('Invalid type  %s.%s.type === %s.%s.type.', schema.name, relName, refModel.name, rel.invRe);
            }
        }
        if (!rel.localFields || !rel.foreignFields) {
            if (rel.type === schema_consts_1.RELATION_TYPE.hasOne) {
                if (rel.aggregationKind === schema_consts_1.AGGREGATION_KIND.none) {
                    ...
                    rel.localFields = ['id'];
                    rel.foreignFields = [rel.invName + 'Id'];
                }
            }

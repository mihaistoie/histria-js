...
async function testLoad() {
    let transaction = new index_1.Transaction();
    let order = await transaction.create(compositions_model_1.Order);
    let item1 = await transaction.load(compositions_model_1.OrderItem, { orderId: order.uuid });
    let item2 = await transaction.load(compositions_model_1.OrderItem, { orderId: order.uuid });
    let children = await order.items.toArray();
    assert.equal(children.length, 2, '(1) Order has 2 items');
    assert.deepEqual(children.map(ii => ii.uuid).sort(), [item1.uuid, item2.uuid].sort(), '(2) Order has 2 items');
    let order2 = await transaction.load(compositions_model_1.Order, { id: 101, items: [{ id: 1, amount: 0 }, { id: 2, amount: 0 }, { id: 3, amount: 0 }] });
    let children2 = await order2.items.toArray();
    assert.equal(children2.length, 3, 'Order has 3 items');
    let oi2 = await transaction.findOne(compositions_model_1.OrderItem, { id: 2 });
...
async function testRestore() {
    let transaction = new index_1.Transaction();
    let order1 = await transaction.load(compositions_model_1.Order, { id: 101, totalAmount: 0, items: [{ id: 1, amount: 0 }, { id: 2, amount: 0 }, { id: 3, amount: 0 }] });

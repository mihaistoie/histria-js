...
function testSales() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let so = yield transaction.create(model_model_1.SalesOrder);
        yield so.setRuleCount(0);
        yield so.netAmount.setValue(100);
        ...
        yield so.grossAmount.setValue(330);
        na = yield so.netAmount.value;
        vat = yield so.vat.value;
        ga = yield so.grossAmount.value;
        ...
        yield so.netAmount.setValue(2000);
        ...
        yield so.netAmount.setValue(920);
        assert.equal(so.$errors.netAmount.error, '', 'No error');
    });
}

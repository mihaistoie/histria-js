...
describe('View Avanced', () => {
    ...
    it('View avanced - autocreate - simple', async () => {
        ...
        const viewOfOrder = await transaction.create(view_avanced_model_1.VAOrderView, { external: true });
    ...
    it('View avanced - autocreate - complex', async () => {
        ...
        const item1 = await transaction.create(view_avanced_model_1.VAOrderItem);
        const item2 = await transaction.create(view_avanced_model_1.VAOrderItem);
        await order.items.add(item1);
        await order.items.add(item2);
        const item1Id = item1.id;
        await viewOfOrder.setOrder(order);
        const viewOfOrderItem = item1.viewOfMe(view_avanced_model_1.VAOrderItemView);
        assert.notEqual(viewOfOrderItem, null, '(1) View of OrderItem found');
        let list = await transaction.find(view_avanced_model_1.VAOrderItemView, {});

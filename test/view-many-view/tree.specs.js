...
async function testCreate() {
    ...
    let child01 = await transaction.create(view_many_view_model_1.Tree);
    let child02 = await transaction.create(view_many_view_model_1.Tree);
    let root01 = await transaction.create(view_many_view_model_1.Tree);
    ...
    let root = await transaction.findOne(view_many_view_model_1.Tree, { id: rootId });
...
async function testLoad() {
    let transaction = new index_1.Transaction();
    let child01 = await transaction.load(view_many_view_model_1.Tree, { id: 102, parentId: 100 });
    let child02 = await transaction.load(view_many_view_model_1.Tree, { id: 101, parentId: 100 });
    let root01 = await transaction.load(view_many_view_model_1.Tree, { id: 100 });
    let children = await root01.leafs.toArray();
    assert.equal(children.length, 2, 'Root has 2 children');
    let data1 = transaction.saveToJson();
    transaction.clear();
    ...
    let data2 = transaction.saveToJson();
    assert.deepEqual(data1, data2, 'Restore test in restore');
    let root = await transaction.findOne(view_many_view_model_1.Tree, { id: 100 });
    children = await root.leafs.toArray();
    assert.equal(children.length, 2, 'Root has 2 children (2)');
    transaction.clear();
}
